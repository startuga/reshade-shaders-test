/*
 * bilateral-contrast-hybrid.fx
 *
 * This shader must be used with the "colour_space.fxh" header file.
 *
 * This is a performance-balanced shader that enhances texture clarity. It uses
 * a fast linear luminance bilateral filter but performs its edge-detection
 * using a perceptually uniform model.
 */

#include "colour_space.fxh" // By lilium (https://github.com/EndlesslyFlowering/ReShade_HDR_shaders)

/**
 * Bilateral Local Contrast Enhancement (Hybrid Quality Edition)
 *
 * Developed by Gemini, reviewed and refined with community feedback.
 *
 * Key Features:
 * - Hybrid approach: Accumulates fast linear luminance but uses a perceptually
 *   uniform model (cube root of luminance) for superior edge detection.
 * - Offers a significant quality improvement over the standard fast version with
...
uniform float SigmaRange <
    ...
    ui_step = 0.01;
    ui_label = "Sigma Range (Perceptual Preservation)";
    ui_tooltip = "Perceptual luminance similarity threshold.\nHigher values smooth more details; lower values preserve more edges.";
> = 0.15;
...
{
    // --- Get Center Luminance (Linear and Perceptual) ---
    float3 linear_rgb;
    ...
    // HYBRID MODEL: Use the cube root of luminance as a fast perceptual approximation.
    float perceptual_center = pow(max(luma_center, 0.0), 1.0/3.0);
    ...
    for (int y = -Radius; y <= Radius; ++y)
    {
        for (int x = -Radius; x <= Radius; ++x)
        {
            ...
            float perceptual_sample = pow(max(luma_sample, 0.0), 1.0/3.0);

            // Perceptual difference for range weight
            float perceptual_diff = perceptual_sample - perceptual_center;
            
            float weight_spatial = exp(-(x*x + y*y) / (2.0 * sigma_spatial_sq));
            float weight_range = exp(-(perceptual_diff*perceptual_diff) / (2.0 * sigma_range_sq));
            float current_weight = weight_spatial * weight_range;
            
            // Linear accumulation for blurring
...
//==============================================================================
technique BilateralContrast_Hybrid
<
    ui_label = "Bilateral Contrast (Hybrid)";
    ui_tooltip = "A performance-balanced shader using a fast linear filter with a perceptual range check.";
>
{
    pass
    {
