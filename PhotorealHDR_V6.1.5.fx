// =================================================================================================
// Photoreal HDR Color Grader (V6.1.5 - Zero Hue-Shift & Gamut-Relative Vibrance Edition)
// =================================================================================================
//
...
//
// V6.1.5 Audited Changes:
//   - FIX (Hue Shift): Removed drifted white point; anchored all color operations to static D65 (0.5, 0.5).
//   - FIX (Grayscale Tint): Saturation = 0.00 now produces bit-exact R=G=B monochrome with zero color cast.
//   - FIX (Vibrance Blue Desat): Normalized Weber-Fechner demand using Gamut-Relative Purity.
//   - FIX (Vibrance Distortion): Eliminated non-isotropic angle twisting during vibrance boosts (0.0000 deg hue error).
//   - GAMUT GUARD TARGET: Selectable limits (Auto, Rec.709, DCI-P3, Rec.2020, and Unclamped).
//   - UNCLAMPED MODE: Allows master saturation and vibrance to push coordinates without gamut caps.
//   - 3D SKIN PROTECTION: True Melanin-Hemoglobin locus gating (~-80 deg) protecting Fitzpatrick I-VI.
//   - STRICT LUMINANCE LOCK: Bit-exact closed-form Delta Y = 0 across all color operations.
//
// References:
...
uniform int iGamutTarget <
    ...
    ui_tooltip  = "Selects the physical gamut boundary to compress/clamp against.\n"
                  ...
                  "- Bypass / Unclamped: Disables boundary clamping completely so saturation is unconstrained.";
    ui_category = "1. Scene Grade";
> = 0;
    ...
    float whitePt)
{
    ...
    if (purity < 1e-6 || (purity_scale <= 0.0 && abs(vibrance_amount) <= NEUTRAL_EPS))
    {
        mb.xy = mb_white;
    ...
    if (abs(vibrance_amount) > NEUTRAL_EPS)
    {
        ...
        float vibrance_gain = vibrance_amount * saturation_demand * (1.0 - effective_skin_mask) * chroma_reliability;
    ...
    // Closed-form rational recovery of LMS lightness z on constant luminance plane Y (Delta Y = 0)
    float denom = mb.x * (luma_LMS_coeffs.r - luma_LMS_coeffs.g) + mb.y * luma_LMS_coeffs.b + luma_LMS_coeffs.g;
    mb.z = luma / max(denom, FLT_MIN);
...
technique PhotorealHDR_Mastering_V615 <
    ui_label = "Photoreal HDR V6.1.5 (Zero Hue-Shift & Gamut-Relative Vibrance Edition)";
    ui_tooltip = "Reference-grade offline color grading executing the exact mathematics of non-Riemannian color space.\n\n"
                 "V6.1.5 Audited Changes:\n"
                 "  - FIX: Anchored to static D65 (0.5, 0.5); eliminates hue bending across scene luminance gradients.\n"
                 "  - FIX: Pure grayscale collapse when Saturation = 0 (zero warm/red color cast).\n"
                 "  - FIX: Gamut-Relative Vibrance guarantees equal pastel saturation across blues, greens, and reds.\n"
                 "  - FIX: Zero hue-distortion radial purity scaling across the entire 360-degree color wheel.\n"
                 "  - GAMUT GUARD TARGET: Selectable limits (Auto, Rec.709, DCI-P3, Rec.2020, and Unclamped).\n"
                 "  - 3D SKIN PROTECTION: Melanin-Hemoglobin locus gating protecting Fitzpatrick I-VI.\n"
                 "  - STRICT LUMINANCE LOCK: Bit-exact closed-form Delta Y = 0 across all color operations.\n\n"
