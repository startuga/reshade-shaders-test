...
uniform float fDarkProtection <
    ...
    ui_min = 0.0;
    ui_max = 0.0025; // A generous but focused range for protecting darks and lower mid-tones.
    ui_step = 0.00001; // FIX: Increased step for better UI responsiveness
> = 0.1;
...
void PS_BilateralContrast(float4 vpos : SV_Position, float2 texcoord : TEXCOORD0, out float4 fragColor : SV_Target)
{
    ...
    if (sum_weight > WEIGHT_THRESHOLD)
    {
        ...
        // FIX: Add numerical stability to logarithmic calculation
        ...
        if (log_base > LUMA_EPSILON) {
            ...
            const float protection_factor = smoothstep(0.0, max(fDarkProtection, LUMA_EPSILON), perceptual_luma);
            luma_diff *= protection_factor;
        } else {
            // Fallback: disable protection if logarithmic calculation would be unstable
            // This should never happen with the current constants, but provides safety
        }
    ...
    // FIX: Ensure no NaN/Inf propagation
    enhanced_linear = min(enhanced_linear, enhanced_linear); // NaN check: NaN != NaN
