...
uniform float GAMUT_COMPRESSION_KNEE <
	ui_type = "slider";
	ui_category = "Tone Mapping";
	ui_min = 0.0;
	ui_max = 0.50;
	ui_step = 0.01;
	ui_label = "Gamut Compression Knee";
	ui_tooltip = "Controls the soft-knee transition width for MacLeod-Boynton gamut boundary compression (0.00 = hard boundary, 0.10 = 10% soft knee).";
> = 0.10;
...
// =================================================================================================
// MACLEOD-BOYNTON (MB) ANALYTICAL SOFT-KNEE GAMUT BOUNDARY COMPRESSION (fGamutGuardKnee)
// =================================================================================================

// Step 2: Ray-traces the target gamut boundary in MacLeod-Boynton space using 16-step bisection.
// Solves for maximum allowable chromaticity purity (P_max) before any RGB component drops below 0.
float SolveGamutBoundaryExact(
		float2 white_mb,
		float2 chroma_dir,
		float3x3 lms_to_rgb_matrix) {
	float t_low = 0.0f;
	float t_high = 8.0f;
	...
	for (int i = 0; i < 16; ++i) {
		...
		float2 xy_t = white_mb + t * chroma_dir;
		
		// Unscaled candidate LMS vector with Y = L + M = 1.0
		float3 lms_test = float3(xy_t.x, 1.0f - xy_t.x, xy_t.y);
		float3 rgb_test = mul(lms_to_rgb_matrix, lms_test);

		if (min(rgb_test.r, min(rgb_test.g, rgb_test.b)) >= 0.0f) {
			t_low = t;
		} else {
			t_high = t;
		}
...
// Analytical soft-knee gamut boundary compression algorithm operating in MacLeod-Boynton space.
// Squeezes oversaturated colors back into the target gamut without hue shifts or luma distortion.
float3 fGamutGuardKnee(
		float3 lms_in,
		float3x3 lms_to_rgb_matrix,
		bool target_bt2020,
		float purity_scale,
		float knee) {
	// Stockman-Sharpe perceived luminance observer weights
	static const float3 LMS_LUMA_WEIGHTS = float3(0.68990272f, 0.34832189f, 0.0f);
	
	// Sanitize and clamp input LMS cone space to non-negative physical light space
	float3 lms_clamped = max(lms_in, 0.0f);
	float Y_orig_luma = dot(lms_clamped, LMS_LUMA_WEIGHTS);

	// D65 neutral reference white point in LMS cone space and MacLeod-Boynton space
	static const float3 LMS_WHITE = float3(1.0632237f, 0.90973593f, 0.57893588f);
	static const float Y_W = 1.97295963f; // L_w + M_w
	static const float2 WHITE_MB = float2(0.53889781f, 0.29343521f); // (x_w, y_w)

	// ---------------------------------------------------------------------------------------------
	// Step 1: Transformation into MacLeod-Boynton (MB) Space
	// ---------------------------------------------------------------------------------------------
	float Y = lms_clamped.x + lms_clamped.y; // Physiological luminance proxy Y = L + M
	if (Y <= 1e-12f || Y_orig_luma <= 1e-12f) {
		float Y_w_luma = dot(LMS_WHITE, LMS_LUMA_WEIGHTS);
		return LMS_WHITE * (max(Y_orig_luma, 0.0f) / Y_w_luma);
	}

	float x = lms_clamped.x / Y; // L-cone chromaticity coordinate
	float y = lms_clamped.z / Y; // S-cone chromaticity coordinate

	float2 chroma_offset = float2(x - WHITE_MB.x, y - WHITE_MB.y);
	float purity = length(chroma_offset);

	if (purity <= 1e-9f) {
		return lms_clamped;
	}

	float2 chroma_dir = chroma_offset / purity;

	// ---------------------------------------------------------------------------------------------
	// Step 2: Ray-Tracing the Gamut Boundary (SolveGamutBoundaryExact)
	// ---------------------------------------------------------------------------------------------
	float P_max = SolveGamutBoundaryExact(WHITE_MB, chroma_dir, lms_to_rgb_matrix);

	// ---------------------------------------------------------------------------------------------
	// Step 3: Core Soft-Knee Compression Formula
	// ---------------------------------------------------------------------------------------------
	float P_thresh = purity * purity_scale;
	float P_compressed = P_thresh;

	if (knee > 0.0f && P_max > 1e-7f) {
		float threshold = P_max * (1.0f - knee);
		if (P_thresh > threshold) {
			float excess = P_thresh - threshold;
			float headroom = P_max - threshold;
			if (headroom > 1e-7f) {
				P_compressed = threshold + headroom * (1.0f - exp(-excess / headroom));
			} else {
				P_compressed = threshold;
			}
		}
	} else {
		P_compressed = min(P_thresh, P_max);
	}

	// Safety Hard-Clamp Fallback
	P_compressed = min(P_compressed, P_max * (1.0f - 1e-6f));

	float2 chroma_offset_compressed = chroma_dir * P_compressed;

	// ---------------------------------------------------------------------------------------------
	// Step 4: Luminance Exact Lock
	// ---------------------------------------------------------------------------------------------
	float x_final = WHITE_MB.x + chroma_offset_compressed.x;
	float y_final = WHITE_MB.y + chroma_offset_compressed.y;

	float3 LMS_trial = float3(x_final * Y, (1.0f - x_final) * Y, y_final * Y);
	float Y_trial = dot(LMS_trial, LMS_LUMA_WEIGHTS);

	float Y_corrected = Y * (Y_orig_luma / max(Y_trial, 1e-37f));

	float3 LMS_final = float3(
		x_final * Y_corrected,
		(1.0f - x_final) * Y_corrected,
		y_final * Y_corrected
	);

	// Final absolute safety check against target RGB negative components
	float3 rgb_check = mul(lms_to_rgb_matrix, LMS_final);
	if (min(rgb_check.r, min(rgb_check.g, rgb_check.b)) < 0.0f) {
		float3 rgb_safe = max(rgb_check, 0.0f);
		if (target_bt2020) {
			LMS_final = mul(BT709_TO_LMS, mul(BT2020_TO_BT709, rgb_safe));
		} else {
			LMS_final = mul(BT709_TO_LMS, rgb_safe);
		}
	}
...
float3 ApplyGamutCompression(float3 bt709, uint output_transfer) {
	...
	// Use if/else instead of ternary operator to avoid X3020 dimension mismatch on float3x3
	float3x3 lms_to_target_rgb;
	if (target_bt2020) {
		lms_to_target_rgb = mul(BT709_TO_BT2020, LMS_TO_BT709);
	} else {
		lms_to_target_rgb = LMS_TO_BT709;
	}

	float3 lms_in = mul(BT709_TO_LMS, bt709);

	// Execute analytical MacLeod-Boynton soft-knee gamut boundary compression
	float3 lms_compressed = fGamutGuardKnee(
			lms_in,
			lms_to_target_rgb,
			target_bt2020,
			1.0f,
	...
	return mul(LMS_TO_BT709, lms_compressed);
}
...
float4 CacheFrameState(
	...
	float3 estimated_peak_white_working;
	if (GRADING_SPACE == SPACE_YF) {
		estimated_peak_white_working = estimated_peak_white;
	} else {
		estimated_peak_white_working = ToWorking(estimated_peak_white, GRADING_SPACE);
	}
...
float4 PresentOutput(
	...
	if (float_target) {
		...
		input = tex2D(SplitIntermediateSampler, texcoord);
	}
