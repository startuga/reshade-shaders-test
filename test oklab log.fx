/**
 * Perceptually Uniform Detail Enhancer v2.2 (Production Ready, HDR-Safe)
 *
 * This version implements a final, critical safety measure for HDR processing by
 * introducing a controlled reconstruction method. This prevents numerical instability
 * and overshoot/undershoot when applying detail enhancement in a logarithmic, unbounded space.
 *
 * Version 2.2:
 *  - [FINAL HDR FIX] Implemented a controlled deviation and reconstruction pipeline.
 *    The shader no longer adds detail directly to the original lightness. Instead, it
 *    calculates the deviation from the blurred base, soft-clamps this deviation using
 *    a new "Overshoot Protection" parameter, and adds it back to the stable base.
 *  - This completely prevents extreme values from reaching the final exp() conversion,
 *    making the algorithm robust against all edge cases and content types.
 *
 * Version 2.1:
 *  - Switched to log-domain processing to handle HDR ranges safely.
 * Version 2.0:
 *  - Switched to the Oklab color space for superior color accuracy.
 */
...
uniform float fStrength <
    ...
    ui_min = 0.0; ui_max = 5.0; ui_step = 0.05; // Increased max for more range
> = 1.0;
...
//----------------------------------------------------------------------------------------------------------------------
static const float3x3 M1 = float3x3(0.4121656120, 0.5362752080, 0.0514575653, 0.2118591070, 0.6807189584, 0.1074065790, 0.0883097947, 0.2818474174, 0.6302613616);
static const float3x3 M2 = float3x3(0.2104542553,  0.7936177850, -0.0040720468, 1.9779984951, -2.4285922050,  0.4505937099, 0.0259040371,  0.7827717662, -0.8086757660);
static const float3x3 M2_inv = float3x3(1.0,  0.3963377774,  0.2158037573, 1.0, -0.1055613458, -0.0638541728, 1.0, -0.0894841775, -1.2914855480);
static const float3x3 M1_inv = float3x3(4.0767245293, -3.3072168827,  0.2307590544, -1.2681437731,  2.6093323231, -0.3411344290, -0.0041119855, -0.7034763098,  1.7068625689);

float3 to_oklab(float3 rgb) { return mul(M2, pow(abs(mul(M1, rgb)), 1.0 / 3.0) * sign(mul(M1, rgb))); }
float3 from_oklab(float3 oklab) { return mul(M1_inv, pow(mul(M2_inv, oklab), 3.0)); }
...
float4 OklabHDRDetailEnhancerPS(float4 vpos : SV_POSITION, float2 texcoord : TEXCOORD) : SV_TARGET
{
    ...
    // 1. Calculate the raw, potentially unstable enhancement.
    //    (Using fStrength here instead of a separate 'boost' is cleaner).
    float enhancedDetail = detailLightness * fStrength;

    // 2. Measure the deviation this enhancement would cause from the stable blurred base.
    //    rawNewLightness = originalLightness + enhancedDetail = blurredLightness + detailLightness + enhancedDetail.
    //    deviation = rawNewLightness - blurredLightness = detailLightness + enhancedDetail.
    float deviation = detailLightness + enhancedDetail;

    // 3. Soft-clamp the deviation using the Overshoot Protection parameter.
    //    This is the key to stability. It limits how far the final result can stray.
    //    The `tanh` function smoothly caps the deviation.
    float safe_deviation = fOvershootProtection * tanh(deviation / (fOvershootProtection + EPSILON_DIV));

    // 4. Construct the final lightness by adding the SAFE deviation to the stable base.
...
technique OklabHDRDetailEnhancer <
    ...
    ui_tooltip = "State-of-the-art detail enhancement using a fully HDR-safe, color-accurate pipeline.\n"
                 "V2.2 - Production-ready with overshoot protection.";
>
{
    pass
    {
