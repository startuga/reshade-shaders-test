// =================================================================================================
// Photoreal HDR Color Grader (V5.9.7-r10 - Production LMS Optimized Non-Riemannian Edition)
// =================================================================================================
//
...
// - Non-Riemannian Color Geometry: Incorporates Bujack et al. 2025 Bezold-Brücke hue stabilization
//   and Naka-Rushton perceptual saturation diminishing returns.
//
// =================================================================================================
...
uniform float fBezoldBrucke <
    ui_type     = "slider";
    ui_min      = 0.00; ui_max = 1.00; ui_step = 0.01;
    ui_label    = "Bezold-Brücke Hue Compensation";
    ui_tooltip  = "Applies a non-linear rotation in MacLeod-Boynton space to counteract perceived\nhue shifts as luminance/exposure is changed (physiological constant-hue tracking).";
    ui_category = "1. Scene Grade";
> = 0.00;
...
float3 ApplyTrolandBleachingLMS(float3 lms, float strength)
{
    ...
    float3 bleached = lerp((0.5 * lm_sum).xxx, lms, k); // Explicit scalar-to-vector promotion
 ...
 */
float3 ApplyMBPurityAndGamutGuardLMS(float3 lms, float purity_scale, float knee, float3 luma_LMS_coeffs, float3x3 to_RGB_boundary, float2 mb_white, float whitePt)
{
    float lm_sum = lms.r + lms.g;
    if (lm_sum <= 0.0)
    {
    ...
    // Optimized: Ad-hoc hard thresholds replaced with the physiologically correct 
    // Naka-Rushton visual receptor-saturation formula. This models the exact 
    // "diminishing returns" of perceived saturation in non-Riemannian color space.
    float2 scaled_chroma_offset = chroma_offset;
    if (abs(purity_scale - 1.0) > NEUTRAL_EPS)
    {
        float S = lerp(1.0, purity_scale, chroma_reliability);
        float compressed_purity;

        [branch]
        if (S >= 1.0)
        {
            // Diminishing returns scaling constant
            float k = 0.85; 
            compressed_purity = (S * purity) / (1.0 + (S - 1.0) * purity * k);
        }
        else
        {
            compressed_purity = S * purity;
        }

        scaled_chroma_offset = chroma_offset * (compressed_purity / max(purity, FLT_MIN));
    }

    float final_purity = SqrtIEEE(dot(scaled_chroma_offset, scaled_chroma_offset));

    [branch]
    if (final_purity > FLT_MIN)
    {
        float angle = atan2(chroma_offset.y, chroma_offset.x);
        
        // 1. Physiological Abney Hue Compensation scaled by Relative Purity (perceived saturation)
        if (fAbneyCorrection > NEUTRAL_EPS)
        {
            float abney_shift = sin(angle - 0.8) * 0.15 * relative_purity * fAbneyCorrection * chroma_reliability;
            angle += abney_shift;
        }

        // 2. Physiological Bezold-Brücke Hue Compensation (Counteracts hue shift under luminance changes)
        // Tracks the local logged intensity offset relative to Reference White.
        if (fBezoldBrucke > NEUTRAL_EPS)
        {
            float l_ratio = luma / max(whitePt, FLT_MIN);
            float log_l = log2(max(l_ratio, FLT_MIN));
            
            // Bezold-Brücke shift vanishes at invariable blue/yellow opponent attractors
            float bb_shift = sin(2.0 * angle - 0.5) * 0.08 * log_l * fBezoldBrucke * chroma_reliability;
            angle -= bb_shift;
        }

        scaled_chroma_offset = float2(cos(angle), sin(angle)) * final_purity;
    }
...
technique PhotorealHDR_Mastering_V597r10 <
    ui_label = "Photoreal HDR V5.9.7-r10 (Non-Riemannian LMS Optimized)";
    ui_tooltip = "Photorealistic grading for SDR and HDR.\n\n"
                 "V5.9.7-r10 changes:\n"
                 "  - Integrated Non-Riemannian Color Geometry based on Bujack et al. 2025 research.\n"
                 "  - Swapped linear saturation scaling with continuous Naka-Rushton receptor compression.\n"
                 "  - Implemented logarithmic Bezold-Brücke physiological hue stabilization in MacLeod-Boynton space.\n"
                 "  - Unified Saturation, Gamut Guard, and Hue-Stabilization pipelines (eliminates LMS-MB round trips).\n"
                 ...
                 "  6. MacLeod-Boynton Unified Saturation & Gamut Guard (Hue-preserving, Abney/Bezold-Brücke compensated, soft-knee)\n\n"
