// =================================================================================================
// Photoreal HDR Color Grader (V5.9.7-r7 - Reviewed / Production Edition)
// =================================================================================================
//
...
//
// V5.9.7-r7 Changes from V5.9.7-r6:
// - Fix: ComputeBlackPointRatio early-out at luma <= FLT_MIN now returns shadowFloor,
//         matching the exact mathematical limit as luma -> 0+ and eliminating a C0 discontinuity.
// - Fix: Stage 3 contrast ratio hard-cap (min(..., 100.0)) replaced with C1-continuous
//         rational soft-cap (threshold 80, asymptote 100) to prevent derivative breaks
//         and mach-banding during extreme shadow lifts.
//
// V5.9.7-r6 Changes from V5.9.7-r5:
// - Fix: ApplyMBPurity now applies the gentler saturation boost (space_comp = 0.90) to ALL
//         gamut spaces, including SDR (sRGB/BT.709). Previously SDR used a more aggressive
//         1.0 multiplier while HDR used 0.90. The gentler curve is now universal, providing
//         consistent vivid-color protection and preventing clipping regardless of output
//         encoding. min_boost_share unified to 0.20 for all spaces.
//
// V5.9.7-r5 Changes from V5.9.7-r4:
// - Fix: ApplyMBPurity gentler boost applied to all HDR/BT.2020-wide gamut spaces (scRGB+PQ).
// - Cleanup: Removed unused GetMBWhite function (superseded by compile-time constant).
// - Cleanup: Clarified ComputeBlackPointRatio comment regarding FLT_MIN early-out.
//
...
uniform float fSaturation <
    ...
    ui_tooltip  = "Strictly isoluminant saturation in physiological MacLeod-Boynton space.\n"
                  ...
                  "All color spaces use a gentler boost curve to prevent gamut boundary clipping.";
    ui_category = "1. Scene Grade";
> = 1.08;
...
uniform int iDebugMode <
    ...
    ui_tooltip  = "Debug visualizations operate on the fully graded output.\n"
                  "All outputs are display-referred [0,1], encoded per color space.\n"
...
//
// The smoothstep toe blends from shadowFloor toward raw over luma ∈ (0, 4×bpNits).
// The early-out at luma <= FLT_MIN prevents division-by-zero and correctly 
// returns the mathematical limit of the smooth toe as luma→0+.
float ComputeBlackPointRatio(float luma, float bpNits, float shadowFloor)
{
    if (bpNits <= FLT_MIN) return 1.0;
    if (luma <= FLT_MIN) return shadowFloor; // Exact mathematical limit as luma -> 0+
...
float3 ApplyMBPurity(float3 color, float purity_scale, float3 lumaCoeffs, float3x3 to_LMS, float3x3 to_RGB, float2 mb_white)
{
    ...
    // Saturated Color Protection:
    // If increasing purity (purity_scale > 1.0), attenuate the boost for colors already
    // near the gamut boundary (purity close to MB_PURITY_PROTECTION_CEILING). This
    // prevents vivid colors from clipping or "breaking" during aggressive saturation boosts.
    // Applied uniformly to all gamut spaces (SDR BT.709 and HDR BT.2020) for consistency.
    if (purity_scale > 1.0)
    {
        ...
        float space_comp      = 0.90;  // Gentler boost for all gamut spaces
        float min_boost_share = 0.20;  // Consistent vivid-color protection floor
...
void PS_PhotorealHDR(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float4 fragColor : SV_Target)
{
    ...
    if (abs(fContrast - 1.0) > NEUTRAL_EPS || abs(fShadows) > NEUTRAL_EPS || abs(fHighlights) > NEUTRAL_EPS)
    {
        ...
        if (absLuma > FLT_MIN)
        {
            ...
            // C1-continuous rational soft-cap to exactly 100.0
            // T = 80.0 (Threshold), C = 100.0 (Ceiling), d = 20.0
            float excess = max(ratio - 80.0, 0.0);
            float capped_ratio = min(ratio, 80.0) + (excess / (1.0 + excess / 20.0));
...
technique PhotorealHDR_Mastering_V597r7 <
    ui_label = "Photoreal HDR V5.9.7-r7 (Production)";
    ui_tooltip = "Photorealistic grading for SDR and HDR.\n\n"
                 "V5.9.7-r7 changes:\n"
                 "  - ComputeBlackPointRatio: early-out now returns shadowFloor, matching the\n"
                 "    mathematical limit as luma -> 0+ and removing a C0 discontinuity.\n"
                 "  - Stage 3 contrast ratio: hard min(100.0) replaced with C1-continuous rational\n"
                 "    soft-cap (threshold 80, asymptote 100) to prevent mach-banding.\n\n"
                 "V5.9.7-r6 changes:\n"
                 "  - ApplyMBPurity: gentler saturation boost (0.90x) and vivid-color protection\n"
                 "    now apply uniformly to ALL gamut spaces (SDR sRGB/BT.709 and HDR).\n"
                 "    Previously SDR used a more aggressive 1.0x curve.\n\n"
                 "V5.9.7-r5 changes:\n"
                 "  - ApplyMBPurity: gentler boost applied to all HDR/BT.2020-wide gamut spaces.\n"
                 "  - Cleanup: Removed unused GetMBWhite function.\n\n"
                 "V5.9.7-r4 bug fix:\n"
