/**
 * PHYSICALLY CORRECT Bilateral Contrast Enhancement - MASTERING EDITION (Real-Time Updated)
 *
 * Design Philosophy: PRECISION OVER PERFORMANCE (with Real-Time Scaling)
 * - True IEEE 754 Math (No fast intrinsics or approximations)
 * - Exact IEC/SMPTE Standard Constants
 * - Bit-Exact Neutrality Logic
 * - Real-Time Scalable Kernels (Sparse Convolution support)
 *
 * Version: 8.1.0 (Feature: Real-Time Performance Presets)
 * Author: startuga
 */
...
uniform int iQualityPreset <
    ui_type = "combo";
    ui_label = "Performance Profile";
    ui_items = "Low (Performance)\0Medium (Balanced)\0High (Quality)\0Ultra (Heavy)\0Reference (Mastering)\0";
    ui_tooltip = "Selects kernel density and precision logic.\n"
                 "- Low: 1/2 density, max radius 12\n"
                 "- Medium: 1/2 density, max radius 24\n"
                 "- High: Full density, max radius 32\n"
                 "- Ultra: Full density, max radius 48\n"
                 "- Reference: Full density, max radius 64, Neumaier Summation";
    ui_category = "Presets";
> = 2;
...
uniform int iRadius <
    ...
    ui_tooltip = "Actual used radius is clamped by the Performance Profile selected.";
    ui_min = 1; ui_max = 64;
...
float3 ProcessPixel(int2 center_pos, float2 texcoord)
{
    ...
    // --------------------------------------------------------------------------
    // Real-Time Preset Logic
    // --------------------------------------------------------------------------
    int radius_cap = 64;
    int sample_step = 1;
    bool use_neumaier = true;

    // Presets configuration: Low, Med, High, Ultra, Reference
    if (iQualityPreset == 0)      { radius_cap = 12; sample_step = 2; use_neumaier = false; } // Low
    else if (iQualityPreset == 1) { radius_cap = 24; sample_step = 2; use_neumaier = false; } // Med
    else if (iQualityPreset == 2) { radius_cap = 32; sample_step = 1; use_neumaier = true; }  // High
    else if (iQualityPreset == 3) { radius_cap = 48; sample_step = 1; use_neumaier = true; }  // Ultra
    else                          { radius_cap = 64; sample_step = 1; use_neumaier = true; }  // Reference

    int base_radius = (iQualityPreset == 4) ? 64 : min(iRadius, radius_cap);
    float sigma_s = (iQualityPreset == 4) ? 4.0 : fSigmaSpatial;
    ...
    // Ensure loop bounds respect the preset cap even after adaptive calc
    ...
    // --------------------------------------------------------------------------
    // Main Loop with Step Optimization
    // --------------------------------------------------------------------------
    [loop]
    for (int y = -max_r; y <= max_r; y += sample_step)
    {
        float y_sq = float(y * y);
        if (y_sq > cutoff_r_sq) continue;
        int x_limit = (int)TrueSqrt(radius_sq_f - y_sq);

        // Align x start to grid based on step
        int x_start = -x_limit;
        int rem = abs(x_start) % sample_step;
        if(rem != 0) x_start += (sample_step - rem);

        [loop]
        for (int x = x_start; x <= x_limit; x += sample_step)
        {
            ...
            if (weight > FLT_MIN) {
                if (use_neumaier) {
                    ...
                    // Standard Summation (Performance)
                    stats_log.x += n_log * weight;
                    stats_sq.x += n_log * n_log * weight;
                    stats_w.x += weight;
                }
...
technique BilateralContrast_Reference <
    ui_label = "Bilateral Contrast v8.1.0 (Real-Time Edition)";
    ui_tooltip = "PHYSICALLY CORRECT Bilateral Contrast\n\n"
                 "Features:\n"
                 "- IEEE 754 True Math (No approximations)\n"
                 "- Real-Time Performance Presets (Low -> Reference)\n"
                 "- Stop-Domain Luminance Processing\n\n"
                 "Usage:\n"
                 "- Use 'High' for 1080p gameplay.\n"
                 "- Use 'Low/Medium' for 4K gameplay.\n"
                 "- Use 'Reference' for screenshots.";
>
{
    pass Main
    {
