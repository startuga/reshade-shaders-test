// =================================================================================================
// Photoreal HDR Color Grader (V6.0.5 - Non-Riemannian Geodesic Spectral Edition)
// =================================================================================================
//
...
// - Exact Ray-Tracing: 24-step unrolled binary search solver for exact boundaries,
//   leveraging scale invariance to eliminate inner-loop divisions and coordinate projections.
...
// - Advanced HVS Crosstalk: Ported psychov_test20's dynamic M-cone crosstalk to curve extreme 
//   spectral coordinates (skies, sunsets) gracefully, preventing harsh boundary clipping.
//
// =================================================================================================
...
// =================================================================================================
    ...
    #error "Photoreal HDR requires ReShade 4.8.0 or newer for Compute/LDS Shader support."
#endif
...
// Standard CIE 170-2:2015 2° Fundamental Observer Weights
static const float3   CIE170_WEIGHTS  = float3(0.68990272, 0.34832189, 1.93485343);
static const float2   MB_WHITE_D65    = float2(0.66450238, 1.86364531); // Normalized to standard D65
...
uniform float fContrastPivot <
    ...
    ui_category = "1. Scene Grade";
> = 0.17677669529; // Exactly ZONE_V (17.68% grey) for perfect zone-contrast alignment
...
uniform float fConeResponseExponent <
    ...
    ui_tooltip  = "Adjusts the steepness of the highlight compression curve (Naka-Rushton exponent).\n"
...
uniform float fHighlightsCurvature <
    ...
    ui_tooltip  = "Decoupled technical shoulder shape of the tone mapper.\n"
                  "Set to 0.0 to enable psychov_test22 Adaptive Reference Shoulder mode.";
    ui_category = "2. Tone Mapping";
> = 0.00;
...
uniform int iColorSpaceOverride <
    ...
    ui_tooltip  = "Leverages ReShade's compiler definitions to auto-detect dynamic backbuffer formats.";
    ui_category = "System";
> = 0;
...
// Added: Precise ITU-R BT.2100 HLG Inverse OETF [1]
float3 HLG_EOTF(float3 x)
{
    ...
    return r * 1000.0; // Map HLG to absolute peak
}

// Added: Precise ITU-R BT.2100 HLG OETF [1]
float3 HLG_OETF(float3 x)
{
    ...
    float3 E = clamp(x / 1000.0, 0.0, 1.0);
    float3 r;
...
{
    [branch] if (space == 4) return HLG_EOTF(encoded); // Fixed: Added HLG decode [1]
...
{
    [branch] if (space == 4) return HLG_OETF(lin); // Fixed: Added HLG encode [1]
...
// Conversions strictly implement the standardized CIE 170-2:2015 2° weights
float3 LMS_to_MB(float3 lms)
{
    float3 wlms = lms * CIE170_WEIGHTS;
    float lum = max(wlms.r + wlms.g, FLT_MIN);
    return float3(wlms.r / lum, wlms.b / lum, lum);
}

// Reconstructs unweighted LMS by cleanly dividing standard weights back
float3 MB_to_LMS(float3 mb)
{
    float3 wlms = float3(mb.x * mb.z, mb.z - (mb.x * mb.z), mb.y * mb.z);
    return wlms / CIE170_WEIGHTS;
}
...
/**
 * GetDriftedWhitePointMB
 *
 * Implements the lightness-dependent neutral axis drift formula, defining the neutral
 * axis geometrically as the closest point to the black apex on equal-lightness manifolds [1].
 */
float2 GetDriftedWhitePointMB(float luma, float whitePt, float2 mb_white_static)
{
...
/**
 * SolveGamutBoundaryExact (Optimized 24-Step Edition)
 *
 * Performs a 24-step unrolled binary search solver for boundary boundaries [1].
 * PROOF OF OPTIMIZATION:
 * Since output RGB signs are completely invariant under multiplication by any strictly
 * positive scalar (Y / denom), we can safely evaluate boundary crossings directly on the
 * unscaled candidate coordinate float3(x, 1.0 - x, y).
 * This eliminates 24 division instructions and 24 coordinate projection calls per pixel.
 * Corrections applied:
 * - Fixed: Converted weighted candidates to unweighted LMS before performing the RGB boundary check [1]
 * - Fixed: Scaled luma coefficients by the standard weights to solve denominator constraints in term of weighted space [1]
 */
float SolveGamutBoundaryExact(float2 chroma_direction, float3 luma_LMS_coeffs, float3x3 to_RGB_boundary, float2 mb_white)
{
    ...
    // Scale the luma coefficients by the standard weights to get weighted luma coefficients [1]
    float3 luma_weighted = luma_LMS_coeffs / CIE170_WEIGHTS;
    ...
    for (int iter = 0; iter < 24; iter++)
    {
        ...
        // Solve using the weighted luma coefficients [1]
        float denom = x * (luma_weighted.r - luma_weighted.g) + y * luma_weighted.b + luma_weighted.g;
        
        if (denom <= FLT_MIN)
        {
            t_high = t; // Non-physical domain (denom <= 0)
        }
        else
        {
            // Convert the weighted LMS test coordinate back to unweighted LMS for the RGB boundary matrix check [1]
            float3 test_lms = float3(x, 1.0 - x, y) / CIE170_WEIGHTS;
            float3 rgb_point = mul(to_RGB_boundary, test_lms);
            float min_rgb = min(min(rgb_point.r, rgb_point.g), rgb_point.b);
            
            // Compiler-agnostic IEEE-754 NaN guard
 ...
 *
 * Simulates cone photopigment bleaching under intense retinal illuminance [1].
 * Preserves the exact, pre-bleach luminance value during the bleaching desaturation [1].
 * OPTIMIZATION: Row-sum normalized working spaces project mul(to_LMS, 1.0) strictly to (1,1,1).
 */
float3 ApplyTrolandBleachingLMS(float3 lms, float strength, float3 luma_LMS_coeffs)
{
    ...
    // Luminance-preserving neutral axis extraction
...
/**
 * ApplyMBPurityAndGamutGuardLMS
 * 
 * Corrections applied:
 * - Solved Bug 2: Re-evaluates mb_white using the original scene luma (luma_adaptation) [1]
 * - Solved Bug 3: Executes unrolled ray-trace boundary solver along final, post-rotation angle [1]
 */
float3 ApplyMBPurityAndGamutGuardLMS(float3 lms, float luma_adaptation, float purity_scale, float knee, float abney_correction, float3 luma_LMS_coeffs, float3x3 to_RGB_boundary, float2 mb_white_static, float whitePt)
{
    if (abs(purity_scale - 1.0) < NEUTRAL_EPS && knee < NEUTRAL_EPS && abney_correction < NEUTRAL_EPS)
        return lms;

    float lm_sum = lms.r + lms.g;
    if (lm_sum <= 0.0) return lms;
    ...
    // --- ADAPTATION FIX: Consistent white point reference matching tonemapping baseline [1]
    ...
    if (purity < FLT_MIN) return lms;

    // Apply hue shifts first
    float angle = atan2(chroma_offset.y, chroma_offset.x);
    float relative_lightness = luma_adaptation / max(whitePt, FLT_MIN);
    ...
    float max_purity_original = SolveGamutBoundaryExact(chroma_offset / purity, luma_LMS_coeffs, to_RGB_boundary, mb_white);
    float relative_purity = saturate(purity / max(max_purity_original, FLT_MIN));
    ...
    // Solve boundary along the true post-rotation direction vector
    float2 shifted_chroma_dir = float2(cos(angle), sin(angle));
    float t_max = SolveGamutBoundaryExact(shifted_chroma_dir, luma_LMS_coeffs, to_RGB_boundary, mb_white);
    ...
    if (knee > FLT_MIN)
    {
        ...
        if (thresh_purity > threshold && threshold > FLT_MIN)
        {
            float excess = thresh_purity - threshold;
    ...
    mb.z = luma * (luma / max(trial_Y, FLT_MIN));
 ...
 * 
 * Corrections applied:
 * - Upgraded: Dual-Stage complementary color gate (evaluates mix shares + opponent ACC coordinates) [1]
 */
float3 ApplyMConeCrosstalkLMS(float3 lms, float strength, float3 lms_before_tm, float3 luma_LMS_coeffs, float2 mb_white_static, float whitePt)
{
    ...
    // BIOLOGICAL CORRECTION: Evaluate the cone-relative drive against the global adaptation
    // state (diffuse scene white in LMS) rather than the pixel's local pre-bleach state.
    ...
    // 1. Cone-relative complementary mix gate [1]
    ...
    float3 mb = LMS_to_MB(lms);
    float2 mb_white = GetDriftedWhitePointMB(original_Y, whitePt, mb_white_static);
    float2 offset = mb.xy - mb_white;

    // 2. Opponent-space CastleCSF ACC gate [1]
    float rg = offset.x;
    float yv = offset.y;
    ...
    if (target_radius > FLT_MIN && m_radius > FLT_MIN)
    {
        ...
        float3 trial_lms = MB_to_LMS(mb);
        float trial_Y = dot(trial_lms, luma_LMS_coeffs);
        mb.z = mb.z * (original_Y / max(trial_Y, FLT_MIN));
        return MB_to_LMS(mb);
    }
...
/**
 * CompressLumaPhysiological (Naka-Rushton Shoulder model - psychov_test20)
 *
 * Applies a continuous Naka-Rushton shoulder using the cone response exponent (n)
 * and highlights curvature exponent (h) to safely saturate at target peak values [1.3.5].
 */
float CompressLumaPhysiological(float Y, float targetPeak, float startComp, float exponent, float highlights)
{
    ...
    float h = max(highlights, 1e-6);
    float h_rcp = 1.0 / h;
    
    // Naka-Rushton/generalized Hill equation curvature
    float x_pow = pow(max(x, FLT_MIN), exponent * h);
    float d_pow = pow(max(d, FLT_MIN), h) - 1.0;
    
    if (d_pow <= 0.0) d_pow = FLT_MIN;
    
    float compressed_x = d * pow(x_pow / max(x_pow + d_pow, FLT_MIN), h_rcp);
    return startComp + compressed_x;
}

/**
 * ApplyNonRiemannianGeodesicToneMapper (NRG-TM - psychov_test20)
 *
 * Performs highlight compression, geodesic hue correction, and opponent channel
 * desaturation directly inside the LMS and MacLeod-Boynton physiological domain [1].
 */
float3 ApplyNonRiemannianGeodesicToneMapper(float3 lms, float targetPeak, float compressionStart, float desatStrength, float coneExponent, float h_input, float3 luma_LMS_coeffs, float2 mb_white_static, float whitePt)
{
    float Y = dot(lms, luma_LMS_coeffs);
    if (Y <= 0.0) return lms;

    // Standardize normalized targetPeak to absolute nits internally
    float targetPeakNits = targetPeak * whitePt;
    float startComp = targetPeakNits * compressionStart;

    // Dynamic curvature from Log recovery input
    float h = h_input;
    float Y_comp = CompressLumaPhysiological(Y, targetPeakNits, startComp, coneExponent, h);

    float ratio = Y_comp / max(Y, FLT_MIN);
    float3 mb = LMS_to_MB(lms);
    
    // Lightness-dependent neutral axis drift
    float2 mb_white = GetDriftedWhitePointMB(Y, whitePt, mb_white_static);
    float2 chroma_offset = mb.xy - mb_white;
    float purity = SqrtIEEE(dot(chroma_offset, chroma_offset));
...
VS_Output VS_PhotorealHDR(uint id : SV_VertexID)
{
    ...
    float3 lumaCoeffs;
    float3x3 to_RGB;
    if (space >= 3) // Fixed: Evaluates correctly for space 4 (HLG) as well as 3 (PQ) [1]
    {
        lumaCoeffs = Luma2020;
...
void PS_PhotorealHDR(VS_Output input, out float4 fragColor : SV_Target)
{
    ...
    [branch]
    if (space >= 3) // Configured matrices properly for spaces 3 (PQ) and 4 (HLG) [1]
    {
    ...
    if (absLuma > FLT_MIN)
    {
        ...
        // C1 continuous blend across the pivot (x = 0) to avoid a first-derivative cusp
    ...
    // Evaluates adaptation luma baseline consistently before technical tonemapping [1]
    float luma_adaptation = dot(lms, input.luma_LMS_coeffs);
    ...
    else if (iToneMapperMode == 2) // Non-Riemannian Geodesic Tone Mapper (NRG-TM)
    {
        ...
        // --- UPGRADE 1: Adaptive Naka-Rushton Shoulder Curvature (Auto-H)
        float h;
        if (abs(fHighlightsCurvature) < NEUTRAL_EPS)
        {
            float peak_over_anchor = max(targetPeak, 1.0 + FLT_MIN);
            float reference_one_side_range_log10 = 3.7 * 0.5; // Kunkel & Reinhard 2010 simultaneous reference
            float actual_above_adaptation_range_log10 = max(log10(peak_over_anchor), FLT_MIN);
    ...
    // Stage 5: Gamut Guard passes luma_adaptation to resolve drifted white point references [1]
    // ---------------------------------------------------------------------------------------------
    // STAGE 5 & 6: PURITY & GAMUT GUARD (UNIFIED CONE CHROMATICITY STAGE)
    // ---------------------------------------------------------------------------------------------

    lms = ApplyMBPurityAndGamutGuardLMS(lms, luma_adaptation, fSaturation, fGamutGuardKnee, fAbneyCorrection, input.luma_LMS_coeffs, to_RGB_boundary, mb_white, whitePt);

    // --- UPGRADE 3: MacLeod-Boynton Purity-Preserving Hue Restoration
    if (fHueRestore > NEUTRAL_EPS)
    {
        float3 mb_src = LMS_to_MB(lms_pre_bleach);
        float2 mb_white_src = GetDriftedWhitePointMB(luma_adaptation, whitePt, mb_white);
        float2 source_offset = mb_src.xy - mb_white_src;

        float3 mb_tgt = LMS_to_MB(lms);
        float2 mb_white_tgt = GetDriftedWhitePointMB(dot(lms, input.luma_LMS_coeffs), whitePt, mb_white);
        float2 target_offset = mb_tgt.xy - mb_white_tgt;
        ...
        if (target_radius > FLT_MIN && source_len > FLT_MIN)
        {
            ...
            mb_tgt.xy = mb_white_tgt + blended_offset;

            float3 trial_lms = MB_to_LMS(mb_tgt);
            float trial_Y = dot(trial_lms, input.luma_LMS_coeffs); 
            mb_tgt.z *= dot(lms, input.luma_LMS_coeffs) / max(trial_Y, FLT_MIN); 
...
technique PhotorealHDR_Mastering_V605 <
    ui_label = "Photoreal HDR V6.0.5 (Non-Riemannian Geodesic Spectral Edition)";
    ui_tooltip = "Reference-grade offline color grading executing the exact mathematics of non-Riemannian color space.\n\n"
                 "V6.0.5 Audited Changes:\n"
                 "  - Fixed: Resolved SolveGamutBoundaryExact weighted-to-unweighted coordinate mismatch and scaled luma denominator coefficients to fix fGamutGuardKnee [1].\n"
                 "  - Fixed: Corrected out-of-scope luma_LMS_coeffs compiler break in Stage 6 Hue Restore block [1].\n"
                 "  - Fully integrated exact CIE 170-2:2015 2-degree fundamental observer weights and standard D65 coordinates.\n"
                 "  - Ported psychov_test22 Auto-H Shoulder mode for dynamically display-adapting compression curvature.\n"
                 "  - Ported psychov_test22 dual-stage ACC opponent and cone mix gates to protect complementary purples in crosstalk.\n"
                 "  - Ported psychov_test22 MacLeod-Boynton purity-preserving hue restoration creative slider.\n"
                 "  - Locked Stage 5 drifted white point adaptation to scene luma, matching Stage 4 baselines.\n"
                 "  - Decoupled creative fHighlights recovery from the technical fHighlightsCurvature tone shoulder shape.\n"
                 "  - Recalculated 24-step unrolled gamut boundary solver along the final, post-rotation hue direction.\n"
                 "  - Fixed BT.2100 HLG encoding and decoding fall-through bugs.\n"
                 "  - Procedural vertex shader scales dynamically for space 4 HLG Rec.2020 container profiles.\n"
                 "  - Optimized PS_OutputToScreen blit with hardware-exact sampler tex2Dfetch coordinates.\n\n"
