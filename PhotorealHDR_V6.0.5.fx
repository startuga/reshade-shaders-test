// =================================================================================================
// Photoreal HDR Color Grader (V6.0.5 - Non-Riemannian Smart Saturation Edition)
// =================================================================================================
//
...
// - Smart Saturation (Vibrance): Non-linear $C^\infty$ smooth low-purity booster with MacLeod-Boynton
//   Cauchy skin-tone protection gate, unbound by hard clipping limits.
...
// -------------------------------------------------------------------------------------------------
static const float SRGB_THRESHOLD_EOTF  = 0.040448236277123205; // Exact matching non-linear threshold
static const float SRGB_THRESHOLD_OETF  = 0.003130668442501796; // Exact matching linear threshold
...
// -------------------------------------------------------------------------------------------------
// Row-Sum-Normalized Matrices (Converted to match Oklab LMS Alignment)
// -------------------------------------------------------------------------------------------------
...
uniform float fSaturation <
    ...
    ui_tooltip  = "Strictly isoluminant linear saturation scaling in physiological MacLeod-Boynton space.\nSet to 1.0 for neutral pass.";
    ui_category = "1. Scene Grade";
> = 1.00;

uniform float fSmartSaturation <
    ui_type     = "slider";
    ui_min      = -1.00; ui_max = 1.00; ui_step = 0.01;
    ui_label    = "Smart Saturation / Vibrance";
    ui_tooltip  = "Non-linear physiological saturation engine.\n"
                  "Intelligently boosts muted, low-purity colors while naturally tapering off near gamut boundaries.\n"
                  "Operates unbound by hard clipping limits.";
    ui_category = "1. Scene Grade";
> = 0.00;
...
uniform float fSkinProtection <
    ...
    ui_label    = "Skin Tone Protection Gate";
    ui_tooltip  = "Applies a physiological MacLeod-Boynton Cauchy filter to protect flesh-tone chromaticity vectors\n"
                  "from oversaturation or unnatural color shifts when Smart Saturation is applied.";
    ui_category = "1. Scene Grade";
> = 0.75;
...
uniform int iColorSpaceOverride <
    ...
    ui_items    = "Auto (Default via ReShade)\0sRGB (SDR)\0scRGB (HDR Linear)\0HDR10 (PQ)\0";
 ...
 * axis geometrically as the closest point to the black apex on equal-lightness manifolds.
 */
...
/**
 * ComputeBlackPointRatio (Chromatically Neutral Intensity Scaling - V6.0.3 design)
 *
 * Computes a smooth, luma-governed subtractive black point scaling factor.
 * Since this is applied as a scalar multiplier on the entire LMS vector, it preserves
 * the chromaticity ratios L/(L+M) and S/(L+M), causing zero gamut modifications or hue drift.
 */
 ...
 *
 * Performs a 24-step unrolled binary search solver for boundary boundaries.
 * Output RGB signs are completely invariant under multiplication by any strictly
 * positive scalar (Y / denom). Evaluating boundary crossings directly on the
 * unscaled candidate coordinate eliminates redundant division instructions.
 */
float SolveGamutBoundaryExact(float2 chroma_direction, float3 luma_LMS_coeffs, float3x3 to_RGB_boundary, float2 mb_white)
{
    ...
    for (int iter = 0; iter < 24; iter++)
    {
        ...
        {
            // Evaluate limits using the unscaled, division-free LMS vector
...
/**
 * ApplyMBPurityAndGamutGuardLMS (V6.0.5 Smart Saturation Edition)
 *
 * Integrates Smart Saturation (Vibrance) with physiological MacLeod-Boynton Cauchy skin-tone
 * protection alongside linear saturation, Abney effect compensation, and soft-knee gamut guarding.
 */
float3 ApplyMBPurityAndGamutGuardLMS(
    float3 lms, 
    float luma_adaptation, 
    float purity_scale, 
    float smart_sat, 
    ...
    float2 mb_white_static, 
    float whitePt)
{
    if (abs(purity_scale - 1.0) < NEUTRAL_EPS && 
        abs(smart_sat) < NEUTRAL_EPS && 
        knee < NEUTRAL_EPS && 
    ...
    // Apply linear saturation scaling
    ...
    // ---------------------------------------------------------------------------------------------
    // SMART SATURATION (VIBRANCE) ENGINE WITH MB CAUCHY SKIN TONE GATE
    // ---------------------------------------------------------------------------------------------
    if (abs(smart_sat) > NEUTRAL_EPS)
    {
        // 1. Non-linear low-purity targeting curve (C-infinity smooth rational polynomial)
        // Max boost at relative_purity = 0, smoothly decays to 0 at gamut boundary (relative_purity = 1)
        float low_purity_weight = ((1.0 - relative_purity) * (1.0 - relative_purity)) / (1.0 + 0.75 * relative_purity);

        // 2. Physiological MacLeod-Boynton Flesh-Tone Protection Gate
        // Target flesh-tone vector in MB space: theta_skin = -0.45 rad, relative purity center = 0.20
        static const float SKIN_THETA         = -0.45;
        static const float SKIN_SIGMA_THETA   = 0.50;
        static const float SKIN_PURITY_CENTER = 0.20;
        static const float SKIN_PURITY_SIGMA  = 0.15;

        // Angle wrapping to [-PI, PI] without intrinsics
        float delta_theta = angle - SKIN_THETA;
        delta_theta = delta_theta - 2.0 * PI * floor((delta_theta + PI) / (2.0 * PI));

        // 4th-order rational Cauchy filters for C-infinity smooth attenuation
        float angle_ratio = delta_theta / SKIN_SIGMA_THETA;
        float angle_gate  = 1.0 / (1.0 + angle_ratio * angle_ratio * angle_ratio * angle_ratio);

        float purity_ratio = (relative_purity - SKIN_PURITY_CENTER) / SKIN_PURITY_SIGMA;
        float purity_gate  = 1.0 / (1.0 + purity_ratio * purity_ratio * purity_ratio * purity_ratio);

        float skin_protection_weight = angle_gate * purity_gate * saturate(skin_protection);

        // 3. Combined Smart Saturation Factor
        float smart_factor = 1.0 + smart_sat * low_purity_weight * (1.0 - skin_protection_weight);
        effective_scale *= max(smart_factor, 0.0);
    }
 ...
 * 
 * Evaluates spectral crosstalk drivers near the spectral boundaries.
 */
float3 ApplyMConeCrosstalkLMS(float3 lms, float strength, float3 lms_before_tm, float3 luma_LMS_coeffs, float2 mb_white_static, float whitePt)
{
    ...
    // 1. Cone-relative complementary mix gate
    ...
    // 2. Opponent-space CastleCSF ACC gate
    float rg = offset.x;
    float yv = offset.y;
...
/**
 * CompressLumaPhysiological (C1-Continuous Generalized Hill Shoulder Model)
 *
 * Implements a continuous, monotonic shoulder curve that guarantees exact first-derivative 
 * smoothness (C1 continuity) at the threshold start boundary for any curvature parameters.
 */
float CompressLumaPhysiological(float Y, float targetPeak, float startComp, float exponent, float highlights)
{
    ...
    // Combine parameters to control transition curvature while maintaining exact C1 continuity
...
float3 ApplyNonRiemannianGeodesicToneMapper(float3 lms, float targetPeak, float compressionStart, float desatStrength, float coneExponent, float h_input, float3 luma_LMS_coeffs, float2 mb_white_static, float whitePt)
{
    ...
    // Curvature is combined downstream inside CompressLumaPhysiological
...
float3 EncodeDebug(float3 debug_out, int space)
{
    ...
    if (space == 3)
    {
        // Map debug 0-1 into a legible HDR range (100-600 nits)
...
void PS_PhotorealHDR(VS_Output input, out float4 fragColor : SV_Target)
{
    ...
    if (iDebugMode == 0 &&
        ...
        abs(fSmartSaturation) < NEUTRAL_EPS && fBleaching < NEUTRAL_EPS && 
        iToneMapperMode == 0 && fAbneyCorrection < NEUTRAL_EPS && 
        fGamutGuardKnee < NEUTRAL_EPS && fMConeCrosstalk < NEUTRAL_EPS && 
        fHueRestore < NEUTRAL_EPS)
    {
        fragColor = src;
        return;
    }
    ...
    else if (space == 2) // scRGB HDR Linear (Rec. 709 primaries, Rec. 2020 boundary guard)
    {
        to_LMS = RGB709_to_LMS;
        to_RGB = LMS_to_RGB709;
        to_RGB_boundary = LMS_to_RGB2020; 
    }
    else // sRGB (SDR) with Rec. 709 primaries and boundaries
    {
        to_LMS = RGB709_to_LMS;
        to_RGB = LMS_to_RGB709;
    ...
    // RESTORATION: Employs uniform intensity-governed scale (V6.0.3 design)
    // This maintains bit-exact chromaticity ratios in LMS and MacLeod-Boynton space,
    // ensuring the color gamut and hues are completely unaffected.
    float bp_ratio = 1.0;
    ...
    // ---------------------------------------------------------------------------------------------
    // STAGE 5 & 6: PURITY, SMART SATURATION & GAMUT GUARD (UNIFIED CONE CHROMATICITY STAGE)
    // ---------------------------------------------------------------------------------------------
    ...
    // MacLeod-Boynton Purity-Preserving Hue Restoration
...
technique PhotorealHDR_Mastering_V605 <
    ui_label = "Photoreal HDR V6.0.5 (Non-Riemannian Smart Saturation Edition)";
    ui_tooltip = "Reference-grade offline color grading executing the exact mathematics of non-Riemannian color space.\n\n"
                 ...
                 "  - Integrated Non-Linear Smart Saturation (Vibrance) inside MacLeod-Boynton cone-opponent space.\n"
                 "  - Formulated C-infinity smooth rational polynomial targeting low-purity coordinates without clipping.\n"
                 "  - Integrated physiological 4th-order rational Cauchy skin-tone protection gate in MB space.\n"
                 "  - Enforced exact LMS isoluminant lock across all smart saturation scaling operations.\n"
                 "  - Restored C1 mathematical continuity to the CompressLumaPhysiological shoulder model.\n"
                 "  - Restored chromatically neutral V6.0.3 multiplicative black point logic (zero gamut footprint variation).\n\n"
