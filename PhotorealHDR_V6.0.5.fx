...
//   - CRITICAL FIX: Rewrote White Balance logic to manipulate LMS cones directly in opponent space.
//     Temperature now shifts L/S cones, Tint shifts L/M/S cones. Luminance is perfectly preserved.
//   - PERFORMANCE: Reduced Gamut Boundary Solver from 24 to 16 steps (matches FP16 precision limits).
//   - LOGIC FIX: Corrected Debug Mode 7 (Negative/WCG) to accurately isolate out-of-gamut channels.
//
// References:
 ...
 *
 * Performs a 16-step unrolled binary search solver for boundary boundaries.
 * 16 steps provide precision beyond FP16 limits while reducing ALU load by 33%.
...
void PS_PhotorealHDR(VS_Output input, out float4 fragColor : SV_Target)
{
    ...
    // V6.0.5 FIX: input.wbScale is now correctly calculated in LMS opponent space
    // ---------------------------------------------------------------------------------------------
    lms *= input.wbScale;
    ...
    else if (iToneMapperMode == 2) // Non-Riemannian Geodesic Tone Mapper (NRG-TM)
    {
        ...
        if (abs(fHighlightsCurvature) < NEUTRAL_EPS)
        {
            ...
            // Replaced legacy log10 with exact bit-equivalent log2 multiplication for dynamic portability
    ...
    if (iDebugMode != 0)
    {
        ...
        else if (iDebugMode == 7)
        {
            ...
            else
            {
                ...
                if (any_neg > 0.0)
                {
                    // V6.0.5 FIX: Directly output the negative channel isolation vector
                    debug_out = neg; 
                }
                else
                {
