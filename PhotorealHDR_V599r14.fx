// =================================================================================================
// Photoreal HDR Color Grader (V5.9.9-r14 - Non-Riemannian Exact Reference Edition)
// =================================================================================================
//
...
// - Separated Pipeline: Linear, hue-preserving scene grading (Stages 1 & 2) isolated from 
//   display-referred geodesic gamut mapping (Stages 4 & 5).
...
uniform int iToneMapperMode <
    ...
    ui_items    = "Bypass\0"
                  "Khronos PBR Neutral\0"
                  "Non-Riemannian Geodesic (NRG-TM)\0"
                  "Neutwo (Reinhard/Naka-Rushton)\0";
    ui_tooltip  = "Select the highlight compression and tone mapping operator.";
    ui_category = "2. Tone Mapping";
> = 0;
...
uniform float fDesaturationStrength <
    ...
    ui_tooltip  = "Controls opponent channel depletion in highlight shoulders.";
    ui_category = "2. Tone Mapping";
> = 0.15;

uniform float fConeExponent <
    ui_type     = "slider";
    ui_min      = 0.50; ui_max = 2.00; ui_step = 0.01;
    ui_label    = "Cone Response Exponent (NRG-TM)";
    ui_tooltip  = "Controls the Naka-Rushton cone response exponent (n).\nHigher = higher contrast, Lower = softer roll-off.";
    ui_category = "2. Tone Mapping";
> = 1.00;
 ...
 *
 * Refactors and consolidates the lightness-dependent neutral axis drift formula.
 * Represents the exact locus of closest points to black under the non-Riemannian framework.
 */
 ...
 */
float SolveGamutBoundaryExact(float2 chroma_direction, float Y, float3 luma_LMS_coeffs, float3x3 to_RGB_boundary, float2 mb_white)
{
    float t_low = 0.0;
...
float3 ApplyMBPurityAndGamutGuardLMS(float3 lms, float purity_scale, float knee, float abney_correction, float3 luma_LMS_coeffs, float3x3 to_RGB_boundary, float2 mb_white_static, float whitePt)
{
    ...
    float t_max = SolveGamutBoundaryExact(chroma_dir, luma, luma_LMS_coeffs, to_RGB_boundary, mb_white);
    ...
    // Dynamic Hue Sensitivity scaled dynamically based on RenoDX PsychoV-17 TCIE1702 D65 boundary parameters
    static const float kMeanD65RayDistance = 0.20139844;
    static const float kMaxD65RayDistance  = 1.02634534;
    static const float kMinSensitivity     = 0.35;
    float long_ray_weight = saturate((t_max - kMeanD65RayDistance) / (kMaxD65RayDistance - kMeanD65RayDistance));
    float hue_sensitivity = lerp(1.0, kMinSensitivity, long_ray_weight);
    ...
    if (abney_correction > NEUTRAL_EPS)
    {
        ...
        float shift = abney_profile * relative_purity * abney_correction * hue_sensitivity * chroma_reliability;
        angle += shift;
    }
...
/**
 * ApplyNakaRushtonLuma
 * 
 * Applies the generalized Naka-Rushton hyperbolic ratio function.
 * Solves for sigma dynamically to ensure gray_in maps exactly to gray_out.
 */
float ApplyNakaRushtonLuma(float Y, float targetPeak, float gray_in, float gray_out, float n)
{
    if (Y <= 0.0) return 0.0;
    
    // Solve for sigma^n such that gray_in maps to gray_out
    // gray_out = targetPeak * (gray_in^n) / (gray_in^n + sigma^n)
    float sigma_n = PowNonNegPreserveZero(gray_in, n) * (targetPeak - gray_out) / max(gray_out, FLT_MIN);
    
    float Y_n = PowNonNegPreserveZero(Y, n);
    float Y_comp = targetPeak * Y_n / max(Y_n + sigma_n, FLT_MIN);
 ...
 * ApplyNeutwoLuma
 * 
 * Applies an algebraic tone mapper (Reinhard/Naka-Rushton hybrid) that maps 
 * gray_in to gray_out and asymptotes exactly at targetPeak.
 */
float ApplyNeutwoLuma(float Y, float targetPeak, float clipNits, float gray_in, float gray_out)
{
    if (Y <= 0.0) return 0.0;
    
    // Generalized rational function: f(Y) = (Y * (Y + A)) / (Y + B)
    // We want f(inf) = targetPeak => A = targetPeak - B
    // We want f(gray_in) = gray_out
    // Solving for B yields:
    float B = gray_in * (gray_in + targetPeak - gray_out) / max(gray_out + gray_in, FLT_MIN);
    float A = targetPeak - B;
    
    float Y_comp = (Y * (Y + A)) / max(Y + B, FLT_MIN);
    
    return min(Y_comp, clipNits);
}
 ...
 */
float3 ApplyNonRiemannianGeodesicToneMapper(float3 lms, float targetPeak, float compressionStart, float desatStrength, float cone_response_exponent, float3 luma_LMS_coeffs, float2 mb_white_static, float whitePt)
{
    ...
    // Exact physiological Naka-Rushton luma compression anchored perfectly at ZONE_V
    float Y_comp = ApplyNakaRushtonLuma(Y, targetPeakNits, 0.17677669529 * whitePt, 0.17677669529 * whitePt, cone_response_exponent);
...
/**
 * ApplyNeutwoToneMapper
 *
 * Performs Neutwo algebraic tone mapping inside the LMS and MacLeod-Boynton physiological domain.
 */
float3 ApplyNeutwoToneMapper(float3 lms, float targetPeak, float3 luma_LMS_coeffs, float2 mb_white_static, float whitePt)
{
    float Y = dot(lms, luma_LMS_coeffs);
    if (Y <= 0.0) return lms;
    ...
    // Standardize normalized targetPeak to absolute nits internally
    float targetPeakNits = targetPeak * whitePt;
    float clipNits = targetPeakNits * 10.0; // Clip ceiling set conservatively above peak
    float gray_in = 0.17677669529 * whitePt;
    float gray_out = 0.17677669529 * whitePt;

    float Y_comp = ApplyNeutwoLuma(Y, targetPeakNits, clipNits, gray_in, gray_out);
    ...
    // Physiological desaturation based on retinal photopigment depletion
    float bleaching = 1.0 / (1.0 + (Y_comp / max(whitePt * 2.0, FLT_MIN)));
    float compressed_purity = purity * lerp(1.0, bleaching, fDesaturationStrength);
...
VS_Output VS_PhotorealHDR(uint id : SV_VertexID)
{
    ...
    // Hoist the dynamic transpose matrix projection out of pixel loop (Sequential TEXCOORD2 bound)
    output.luma_LMS_coeffs = mul(lumaCoeffs, to_RGB);

    // Hoist the entire constant white-balance calculation
...
void PS_PhotorealHDR(VS_Output input, out float4 fragColor : SV_Target)
{
    ...
    else if (iToneMapperMode == 2) // Non-Riemannian Geodesic Tone Mapper (NRG-TM)
    {
        ...
        lms = ApplyNonRiemannianGeodesicToneMapper(lms, targetPeak, fCompressionStart, fDesaturationStrength, fConeExponent, input.luma_LMS_coeffs, mb_white, whitePt);
    ...
    else if (iToneMapperMode == 3) // Neutwo (Algebraic Naka-Rushton/Reinhard style)
    {
        float targetPeak = (space <= 1) ? 1.0 : max(1.0, fDisplayPeakNits / whitePt);
        float3 lms_before = lms;
        
        lms = ApplyNeutwoToneMapper(lms, targetPeak, input.luma_LMS_coeffs, mb_white, whitePt);
...
technique PhotorealHDR_Mastering_V599r14 <
    ui_label = "Photoreal HDR V5.9.9-r14 (Non-Riemannian Exact Reference Edition)";
    ui_tooltip = "Reference-grade offline color grading executing the exact mathematics of non-Riemannian color space.\n\n"
                 "V5.9.9-r14 changes:\n"
                 "  - Fixed compilation issue by implementing ApplyNakaRushtonLuma and ApplyNeutwoLuma functions.\n"
                 "  - Removed the redundant LMS709_to_RGB2020 matrix and reverted the scRGB gamut boundary selection back to LMS_to_RGB2020.\n"
                 "  - Fixed dynamic white point drift dead-code in SolveGamutBoundaryExact by aligning parameters to the already drifted white point.\n"
                 "  - Consolidated the lightness-dependent neutral axis drift formula into GetDriftedWhitePointMB to eliminate duplicated logic.\n"
                 "  - Standardized targetPeak parameter signatures across both tone mapping paths to prevent unit desynchronization.\n"
                 "  - Fully parameterized fAbneyCorrection inside ApplyMBPurityAndGamutGuardLMS to clean global coupling.\n"
                 "  - Aligned Contrast Pivot default UI parameter precisely to ZONE_V (17.68% grey) to preserve mathematical consistency.\n"
                 "  - Sequentialized vertex output semantics by renumbering input.luma_LMS_coeffs to TEXCOORD2.\n"
                 "  - Clarified UI option to Labeled 'Auto (Default via ReShade)' for color space overrides.\n"
