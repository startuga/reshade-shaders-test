 ...
 *
 * Version: 8.3.4 (Fix: Rec.2020 Matrix, Luminance Independence, Oklab Bilateral Weight)
 * Author: startuga
 */
...
// Derived from Ottosson M1 × BT.2020 RGB→XYZ_D65, then normalized per row
...
uniform float fSigmaChroma <
    ...
    ui_tooltip = "Controls filter sensitivity to Oklab chromaticity differences.\n"
                 ...
                 "[Changed in v8.3.3: operates in Oklab, not R/Y ratio]";
    ui_category = "Filter Parameters";
> = 0.15;
...
// [v8.3.4 Fix] Replaced ternary matrix selection with if/else to fix X3020
float2 GetOklabChroma(float3 linearRGB, int activeSpace)
{
    ...
    float3 lms_p;
    lms_p.x = sign(lms.x) * pow(max(abs(lms.x), FLT_MIN), 0.333333333);
    lms_p.y = sign(lms.y) * pow(max(abs(lms.y), FLT_MIN), 0.333333333);
    lms_p.z = sign(lms.z) * pow(max(abs(lms.z), FLT_MIN), 0.333333333);
...
float FetchPerceptualLuma(int2 pos)
{
    ...
    // [v8.3.2 Fix] Raised floor from FLT_MIN to 1e-4 (-13 stops)
    // Prevents log2 noise explosion in deep blacks which was breaking edge detection.
    return (log2(max(luma, EDGE_LUMA_FLOOR)) + 20.0) * 0.06;
}
...
float StructureTensor(int2 center)
{
    ...
    // [v8.3.2 Fix] Return Squared Energy (lambda1) instead of Linear Magnitude (sqrt)
    // This matches the units of Sobel/Scharr (gx^2 + gy^2)
    return lambda1 * (1.0 + coherence) * 0.5;
}

// [v8.3.1 Feature] Oklab Perceptual Chroma Edge Detection
// [v8.3.2 Fix] Return Squared Energy (36.0 factor)
float ChromaEdge(int2 center)
{
    int space = (iColorSpaceOverride > 0) ? iColorSpaceOverride : BUFFER_COLOR_SPACE;
    float2 center_ab = GetOklabChroma(FetchLinear(center), space);
    ...
    [unroll] for (int y = -1; y <= 1; y++) {
        [unroll] for (int x = -1; x <= 1; x++) {
            ...
            float2 neighbor_ab = GetOklabChroma(FetchLinear(center + int2(x, y)), space);
            float2 diff = center_ab - neighbor_ab;
            
            // Squared Euclidean distance in Oklab Chroma plane
            maxChromaDiff = max(maxChromaDiff, dot(diff, diff));
        }
    }
    
    // ChromaEdge - recalibrated for normalized (a/L, b/L)
    return maxChromaDiff * 12.0;  // was 36.0, reduced by ~L² geometric mean correction
}
...
int GetAdaptiveRadius(int2 center, int base_radius, float strength, float sigma_spatial)
{
    ...
    // Smoothstep maps squared energy to 0..1 factor
    // Sensitivity tuned for energy inputs (approx 0.001 - 0.01 range)
...
float3 ProcessPixel(int2 center_pos)
{
    ...
    // [v8.3.3 Fix] Full Oklab integration for bilateral weighting.
    // Normalized a/b chroma prevents luminance contamination.
    ...
    if (bChromaAwareBilateral) {
        ...
        center_chroma = GetOklabChroma(color_lin, space);
    }
    ...
    for (int y = y_start; y <= y_end; ++y)
    {
        ...
        for (int x = x_start; x <= x_end; ++x)
        {
            ...
            if (chroma_weight_factor > 0.0 && n_luma > FLT_MIN) {
                float2 n_chroma = GetOklabChroma(n_lin, space);
                float2 d_chroma = center_chroma - n_chroma;
                // [v8.3.3 Fix] Oklab Euclidean distance instead of R/Y ratio
...
technique BilateralContrast_Reference <
    ui_label = "Bilateral Contrast v8.3.4 (Mastering Edition)";
    ui_tooltip = "MASTERING QUALITY - True Math Processing\n\n"
                 "v8.3.3 Fixes:\n"
                 "- Fix: Rec.2020 Oklab Matrix (Correct Derivation)\n"
                 "- Fix: Luminance-Independent Chromaticity (a/L, b/L)\n"
                 "- Fix: Bilateral loop now uses Oklab distance instead of R/Y\n"
                 "- Fix: Consistent Edge Energy Units (Squared Energy)\n"
                 "- Fix: Shadow Noise Floor (1e-4)\n"
                 "- Feature: Oklab Perceptual Chroma Edge Detection\n"
                 "- Feature: Soft Knee Gamut Mapping\n"
