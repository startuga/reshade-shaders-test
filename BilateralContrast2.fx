...
> = 0.15;
    ...
    uniform float _ <
        ui_type = "text";
        ui_text = "Using High-Quality Sampling Mode (25 Samples)";
    >;
#else
    uniform float _ <
        ui_type = "text";
        ui_text = "Using Standard Sampling Mode (up to 1089 Samples)";
    >;
#endif
...
static const float PQ_M1 = 0.1593017578125, PQ_M2 = 78.84375;
static const float PQ_C1 = 0.8359375, PQ_C2 = 18.8515625, PQ_C3 = 18.6875;
...
{
    // 1. Sample and convert the center pixel to linear space.
...
#if HIGH_QUALITY_SAMPLING
    // --- High-Quality Spiral Sampling (25 Samples) ---
    const float2 offsets[24] = {
        float2(0, 1), float2(1, 1), float2(1, 0), float2(1, -1), float2(0, -1), float2(-1, -1), float2(-1, 0), float2(-1, 1),
...
#elif BUFFER_COLOR_SPACE == 3 // HDR10 PQ
    return float4(LinearToPQ(new_linear_rgb), color_in.a);
#elif BUFFER_COLOR_SPACE == 4 // HDR10 HLG
