// =================================================================================================
// Photoreal HDR Color Grader (V5.9.5 - The Ultimate Director's Cut)
// =================================================================================================
//
...
// - Added: Debug Visualization System (9 modes). See iDebugMode uniform for details.
//          Debug outputs are display-linear [0,1] values, correctly encoded per color space
//          (sRGB: gamma OETF, scRGB: linear passthrough, PQ: scaled to 1000 nits).
// - Fix: SRGB_THRESHOLD_OETF unified to derived value (0.04045/12.92) for bit-exact
//        Encode↔Decode round-trip continuity with companion shader (Bilateral Contrast v8.4.5).
//        Previous value (0.0031308) followed the IEC standard's explicit linear-side constant
//        but was inconsistent with the encoded-side 0.04045 at the 5th decimal digit.
// V5.9.4 Changes from V5.9.3:
// - Fix: TROLAND_HALF_SAT calibrated from 20000 → 8000 for visible effect at 1000 nits.
// - Fix: Unified all L+M guards to <= 0.0.
// - Fix: Restored all V5.9.2 slider tooltips, tonal EQ math docs, technique pipeline listing.
// - Fix: Corrected C2 discontinuity documentation (f''(0⁻) = fShadows, not S/2).
// - Fix: Fast bypass simplified fBleaching check (ui_min=0.0, abs unnecessary).
// V5.9.3 Changes from V5.9.2:
...
// - Removed: MacLeod-Boynton Hue Restoration. Proven no-op for Khronos (affine invariant).
// V5.9.1 Changes from V5.9:
// - Fix: Restored vivid-color protection in ApplyMBPurity.
// - Fix: Replaced all rsqrt()/sqrt() intrinsics with IEEE 754 equivalents.
// =================================================================================================
...
// IEC 61966-2-1 specifies the linear-side threshold as 0.0031308 explicitly,
// but this is inconsistent with the encoded-side 0.04045 at the 5th decimal
// digit. Using the derived value guarantees bit-exact Encode↔Decode continuity
...
/// @brief Encodes a display-linear [0,1] debug value for the target color space.
/// Follows the same convention as Bilateral Contrast v8.4.5:
///   sRGB:  gamma OETF applied to saturated [0,1] values
///   scRGB: linear passthrough (buffer is already float)
///   PQ:    mapped to 0-1000 nits for visible PQ encoding
...
/// @brief Zone classification. Aligned with Bilateral Contrast v8.4.5.
...
/// @brief Zone colors. Aligned with Bilateral Contrast v8.4.5.
float3 GetZoneColor(int index)
{
    [flatten]
    switch (clamp(index, 0, 12))
    {
        case 0:  return float3(0.5,  0.0,  0.5);   // Negative: purple
        case 1:  return float3(0.02, 0.02, 0.05);   // Zone I:   near-black
        case 2:  return float3(0.1,  0.0,  0.1);    // Zone II:  deep shadow
        case 3:  return float3(0.2,  0.0,  0.3);    // Zone III: shadow
        case 4:  return float3(0.3,  0.0,  0.5);    // Zone IV:  dark midtone
        case 5:  return float3(0.2,  0.2,  0.8);    // Zone V:   middle grey
        case 6:  return float3(0.5,  0.5,  0.5);    // Zone VI:  light midtone
        case 7:  return float3(0.8,  0.8,  0.2);    // Zone VII: highlight
        case 8:  return float3(1.0,  0.8,  0.3);    // Zone VIII: bright
        case 9:  return float3(1.0,  0.6,  0.4);    // Zone IX:  near-white
        case 10: return float3(1.0,  0.9,  0.8);    // Zone X:   white
        case 11: return float3(1.0,  1.0,  1.0);    // Zone XI:  above white
        case 12: return float3(1.0,  1.0,  0.5);    // Zone XII: far above white
        default: return float3(0.0,  0.0,  0.0);
    }
}

/// @brief False-color heatmap for stop values. Maps -8..+8 stops to a 5-color gradient.
float3 StopsToFalseColor(float stops)
{
    float t = saturate((stops + 8.0) / 16.0); // [0, 1]

    if (t < 0.2)       return float3(0.0, 0.0, t / 0.2);                       // black → blue
    else if (t < 0.4)  return float3(0.0, (t - 0.2) / 0.2, 1.0 - (t - 0.2) / 0.2); // blue → cyan
    else if (t < 0.6)  return float3((t - 0.4) / 0.2, 1.0, 0.0);               // cyan → green → yellow
    else if (t < 0.8)  return float3(1.0, 1.0 - (t - 0.6) / 0.2, 0.0);         // yellow → red
    else               return float3(1.0, (t - 0.8) / 0.2, (t - 0.8) / 0.2);   // red → white
}

/// @brief Converts a hue angle [0,1] to an RGB color wheel value.
float3 HueToRGB(float hue)
{
    ...
    switch (int(i) % 6)
    {
        case 0:  return float3(1.0, t,   p);
        case 1:  return float3(q,   1.0, p);
        case 2:  return float3(p,   1.0, t);
        case 3:  return float3(p,   q,   1.0);
        case 4:  return float3(t,   p,   1.0);
        case 5:  return float3(1.0, p,   q);
        default: return float3(1.0, 1.0, 1.0);
    }
}

/// @brief Computes the bleaching factor k for a given color (0=fully bleached, 1=untouched).
...
/// @brief Computes the Khronos compression ratio for a given color (1.0=none, <1.0=compressed).
...
void PS_PhotorealHDR(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float4 fragColor : SV_Target)
{
    ...
    // ---------------------------------------------------------------------------------------------
    // Save pre-bleach state for debug
    ...
    // ---------------------------------------------------------------------------------------------
    // Save pre-Khronos state for debug
    ...
    if (iDebugMode != 0)
    {
        ...
        // Mode 1: Luminance (False Color Stops)
        // Shows log2(luma / whitePt) as a false-color heatmap.
        // -8 stops = deep blue, 0 = green/yellow, +8 stops = white.
        if (iDebugMode == 1)
        {
            float luma = dot(color, lumaCoeffs);
        ...
        // Mode 2: Zone Map
        // Same zone colors as Bilateral Contrast v8.4.5.
        ...
        // Mode 3: Bleaching Factor
        // k = 1.0 (blue, untouched) → k = 0.0 (red, fully bleached).
        // Computed from pre-bleach state so it shows what the bleaching DID,
        // not what the saturation slider did after.
        ...
        // Mode 4: MB Purity
        // Distance from D65 white in MB chromaticity space.
        // 0 = neutral (black), 0.35+ = gamut boundary (bright).
        // Scaled by 3.0 for visibility (0.35 → ~1.0).
        ...
        // Mode 5: MB Hue Wheel
        // Maps atan2(Δs, Δl) to a color wheel.
        // Brightness modulated by purity: achromatic = dark, saturated = bright.
        // Hue shifts from any operation appear as color discontinuities.
        ...
        // Mode 6: LMS Cone Response
        // L = red, M = green, S = blue.
        // Normalized to max channel for visibility.
        // Useful for verifying white balance: neutral objects should show equal L≈M.
        ...
        // Mode 7: Negative / WCG
        // Highlights out-of-gamut channels:
        //   R < 0 → magenta, G < 0 → cyan, B < 0 → yellow.
        //   All ≥ 0 → dark green (in-gamut).
        //   Any NaN/Inf → white.
        else if (iDebugMode == 7)
        {
            ...
            else
            {
                ...
                if (any_neg > 0.0)
                {
                    // R negative → magenta (1,0,1), G negative → cyan (0,1,1), B negative → yellow (1,1,0)
                    debug_out = float3(
                        max(neg.r, neg.b),
                        max(neg.g, neg.b),
                        max(neg.r, neg.g)
                ...
                {
                    debug_out = float3(0.0, 0.15, 0.0); // In-gamut: dark green
                }
            }
        }
        // Mode 8: Compression Map
        // Shows Khronos compression ratio: 1.0 = no compression (blue), <1.0 = compressed (red).
        // Computed from pre-Khronos state in normalized domain.
        // Only meaningful when Khronos is enabled.
        else if (iDebugMode == 8)
        {
            ...
            {
                debug_out = float3(0.2, 0.2, 0.2); // Khronos disabled: gray
            }
        }
...
technique PhotorealHDR_Mastering_V595 <
    ui_label = "Photoreal HDR V5.9.5 (Director's Cut)";
