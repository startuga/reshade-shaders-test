...
{
    float4 originalSample = tex2D(ReShade::BackBuffer, texcoord);
    float3 premultipliedColor = originalSample.rgb;
    float originalAlpha = originalSample.a;

    // 1. Un-premultiply to get straight alpha color for processing
    float3 straightColor = premultipliedColor;
    if (originalAlpha > EPSILON_DIV)
    {
        straightColor /= originalAlpha;
    }
    
    // --- The entire shader logic now operates on 'straightColor' ---
    
    float originalLuma = dot(straightColor, LUMA_WEIGHTS);
    ...
    for (int y = -iRadiusPixels; y <= iRadiusPixels; ++y)
    {
        ...
        {
            // Sample neighbor and un-premultiply it as well for correct luma comparison
            float2 pixelOffset = float2(x, y);
            float2 sampleTexcoord = texcoord + pixelOffset * ReShade::PixelSize;
            float4 neighborSample = tex2D(ReShade::BackBuffer, sampleTexcoord);

            float3 neighborStraightColor = neighborSample.rgb;
            if (neighborSample.a > EPSILON_DIV)
            {
                neighborStraightColor /= neighborSample.a;
            }
            
            float neighborLuma = dot(neighborStraightColor, LUMA_WEIGHTS);
    ...
    // --- Debug View (operates on straight-alpha domain) ---
    if (bDebugView)
    {
        // Debug view doesn't need premultiplication, it's a diagnostic tool.
    ...
    float3 finalStraightColor = straightColor; // Start with the un-premultiplied color
    if (originalLuma > 0.0f)
    {
        finalStraightColor *= (newLuma / originalLuma);
    }
    else
    {
        finalStraightColor = float3(newLuma, newLuma, newLuma);
    }

    // 3. Re-premultiply the final color before output
    float3 finalPremultipliedColor = finalStraightColor * originalAlpha;

    return float4(finalPremultipliedColor, originalAlpha);
}
