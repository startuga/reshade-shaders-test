// =================================================================================================
// Photoreal HDR Color Grader (V6.0.2 - Non-Riemannian Geodesic Spectral Edition - Audited)
// =================================================================================================
//
...
uniform float fHighlightsCurvature <
    ...
    ui_tooltip  = "Decoupled technical shoulder shape of the tone mapper. Left = Softer; Right = Harder roll-off.";
    ui_category = "2. Tone Mapping";
> = 0.00;
...
float3 ApplyMBPurityAndGamutGuardLMS(float3 lms, float luma_adaptation, float purity_scale, float knee, float abney_correction, float3 luma_LMS_coeffs, float3x3 to_RGB_boundary, float2 mb_white_static, float whitePt)
{
    ...
    // --- RAY-TRACE FIX: Solve boundary along the true post-rotation direction vector [1]
...
/**
 * ApplyMConeCrosstalkLMS (ADVANCED HVS SPECTRAL HUE BENDING - V6.0.1)
 *
 * Models human L/M and S/M overlap (crosstalk / confusion) near the spectral boundaries [1.3.7a].
 * Biases the final target MB hue direction gracefully toward pure M (green/yellow) in
 * regions with extremely high L-dominant or S-dominant cone-relative drives [1.3.7a].
 * Gated by complementary spectral confidence to prevent distortion of purple complementary colors [1.3.7a].
 */
float3 ApplyMConeCrosstalkLMS(float3 lms, float strength, float3 lms_before_tm, float3 luma_LMS_coeffs, float2 mb_white_static, float whitePt)
{
    ...
    // Compute direct crosstalk contribution
    float m_support = kLMConfusion * l_over_m + kSMConfusion * s_over_m;

    // Guard against complementary purple/magenta coordinates
    float3 mb = LMS_to_MB(lms);
    float2 mb_white = GetDriftedWhitePointMB(original_Y, whitePt, mb_white_static);
    float2 offset = mb.xy - mb_white;

    float chroma = max(length(offset), 1e-6);
    float rg_pos = saturate(offset.x / chroma);
    float yv_pos = saturate(offset.y / chroma);
    float mixed = saturate(min(abs(offset.x), abs(offset.y)) / max(max(abs(offset.x), abs(offset.y)), 1e-6));
    float purple_gate = saturate(2.f * rg_pos * yv_pos * mixed);
    float spectral_confidence = 1.0 - purple_gate;

    float3 bent_drive = drive;
    bent_drive.y += saturate(strength) * spectral_confidence * m_support;

    // Preserving dominant-cone magnitude
    float old_max = max(drive.x, max(drive.y, drive.z));
    float new_max = max(bent_drive.x, max(bent_drive.y, bent_drive.z));
    bent_drive *= (old_max / max(new_max, FLT_MIN));

    float3 bent_lms = sign(lms) * bent_drive * global_white_lms;

    // Hue-bending MB reconstruction
    float3 mb_bent = LMS_to_MB(bent_lms);
    float2 bent_offset = mb_bent.xy - mb_white;
    float bent_offset2 = dot(bent_offset, bent_offset);

    if (bent_offset2 > 1e-7)
    {
        float target_radius = length(offset);
        float2 bent_dir = bent_offset * rsqrt(bent_offset2);

        mb.xy = mb_white + bent_dir * target_radius;
        
        // Exact Luminance Lock
...
void PS_PhotorealHDR(VS_Output input, out float4 fragColor : SV_Target)
{
    ...
    [branch]
    if (space >= 3) // Fixed: Configured matrices properly for spaces 3 (PQ) and 4 (HLG) [1]
    {
    ...
    else if (iToneMapperMode == 2) // Non-Riemannian Geodesic Tone Mapper (NRG-TM)
    {
        ...
        // NRG-TM technical curvature controlled via independent highlights exponent [1]
        lms = ApplyNonRiemannianGeodesicToneMapper(lms, targetPeak, fCompressionStart, fDesaturationStrength, fConeResponseExponent, fHighlightsCurvature, input.luma_LMS_coeffs, mb_white, whitePt);
    ...
    // ---------------------------------------------------------------------------------------------
    // STAGE 6a: ADVANCED HVS SPECTRAL M-CONE CROSSTALK (Warm/Spectral Hue Bending)
    // ---------------------------------------------------------------------------------------------
...
technique PhotorealHDR_Mastering_V602 <
    ui_label = "Photoreal HDR V6.0.2 (Non-Riemannian Geodesic Spectral Edition)";
    ui_tooltip = "Reference-grade offline color grading executing the exact mathematics of non-Riemannian color space.\n\n"
                 "V6.0.2 Audited Changes:\n"
