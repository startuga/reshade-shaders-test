// =================================================================================================
// Photoreal HDR Color Grader (V6.0.6 - Non-Riemannian Geodesic Spectral Edition)
// =================================================================================================
//
...
// - Exact Ray-Tracing: 16-step unrolled binary search solver for exact boundaries,
...
// - Advanced HVS Crosstalk: Dynamic M-cone crosstalk to curve extreme spectral coordinates 
//   (skies, sunsets) gracefully, preventing harsh boundary clipping.
//
// V6.0.6 Audited Changes:
//   - PIPELINE FIX: Reordered Stage 5 & 6 so that Hue Restore and M-Cone Crosstalk execute BEFORE
//     Gamut Guard. This guarantees that restored hues cannot violate physical gamut boundaries.
//   - COLOR SCIENCE: Added full ITU-R BT.2100 HLG (Color Space 4) encode, decode, and matrix paths.
//   - PERFORMANCE: Guarded the pre-rotation boundary solver behind Abney Correction check,
//     saving 16 binary search iterations per pixel when Abney Correction is at default (0.0).
//   - COMPATIBILITY: Added explicit VertexCount and PrimitiveTopology definitions to pass block.
//   - DEBUG FIX: Extended EncodeDebug to support HLG display visualization.
//
// References:
...
// Note: These matrices are explicitly normalized so that D65 (1,1,1 in RGB) maps to (1,1,1) in LMS.
// -------------------------------------------------------------------------------------------------
...
// Added: Precise ITU-R BT.2100 HLG Inverse OETF (1000 Nits Reference Scene Mapping)
...
// Added: Precise ITU-R BT.2100 HLG OETF
...
/**
 * SolveGamutBoundaryExact (Optimized 16-Step Edition)
 *
 * Performs a 16-step unrolled binary search solver for boundary crossings.
 * 16 steps provide precision beyond FP16 limits while reducing ALU load.
...
float SolveGamutBoundaryExact(float2 chroma_direction, float3 luma_LMS_coeffs, float3x3 to_RGB_boundary, float2 mb_white)
{
    ...
    [unroll]
    for (int iter = 0; iter < 16; iter++) // Reduced from 24 for performance (FP16 precision limit)
    {
...
/**
 * ApplyMBPurityAndGamutGuardLMS (Unified Final Boundary Guard)
 */
float3 ApplyMBPurityAndGamutGuardLMS(float3 lms, float luma_adaptation, float purity_scale, float knee, float abney_correction, float3 luma_LMS_coeffs, float3x3 to_RGB_boundary, float2 mb_white_static, float whitePt)
{
    ...
    // PERFORMANCE OPTIMIZATION: Only evaluate pre-rotation boundary if Abney Correction is active
    ...
    float max_purity = SolveGamutBoundaryExact(shifted_chroma_dir, luma_LMS_coeffs, to_RGB_boundary, mb_white);
...
VS_Output VS_PhotorealHDR(uint id : SV_VertexID)
{
    ...
    float3 lumaCoeffs;
    float3x3 to_RGB;
    if (space >= 3) // Spaces 3 (PQ) and 4 (HLG) use Rec.2020 primaries
    {
        lumaCoeffs = Luma2020;
    ...
    // V6.0.5 FIX: Hoist White Balance calculation in LMS Opponent Domain
    // L-cone: Warm (+) / Magenta (+)
    // M-cone: Green (+) / Magenta (-)
    // S-cone: Cool (+) / Magenta (+)
    ...
    // Normalize to preserve luminance of a neutral reference white (D65 maps to 1,1,1 in LMS)
...
void PS_PhotorealHDR(VS_Output input, out float4 fragColor : SV_Target)
{
    ...
    // ---------------------------------------------------------------------------------------------
    // DESIGN INTENT NOTE:
    // scRGB (space == 2) natively uses BT.709 primaries and matrices to encode coordinates. 
    // Since scRGB is floating-point, it permits values outside [0,1] to represent wide gamuts.
    // The design intentionally maps this container's boundaries to Rec. 2020 (LMS_to_RGB2020) 
    // inside the unrolled gamut guard to restrict ultimate saturation outputs to physical display 
    // bounds.
    // ---------------------------------------------------------------------------------------------
    [branch]
    if (space >= 3) // HDR10 (PQ) & HLG with Rec. 2020 primaries
    {
    ...
    // Note: Must execute prior to Gamut Guard so that restored vectors are bounded cleanly.
    // ---------------------------------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------------------------------
    // STAGE 6: PURITY & GAMUT GUARD (UNIFIED CONE CHROMATICITY FINAL GATE)
    // ---------------------------------------------------------------------------------------------
...
technique PhotorealHDR_Mastering_V606 <
    ui_label = "Photoreal HDR V6.0.6 (Non-Riemannian Geodesic Spectral Edition)";
    ui_tooltip = "Reference-grade offline color grading executing the exact mathematics of non-Riemannian color space.\n\n"
                 "V6.0.6 Audited Changes:\n"
                 "  - PIPELINE FIX: Reordered Stage 5 & 6 so that Hue Restore and M-Cone Crosstalk execute BEFORE\n"
                 "    Gamut Guard, ensuring restored hues cannot violate physical boundaries.\n"
                 "  - COLOR SCIENCE: Full ITU-R BT.2100 HLG (Color Space 4) encode/decode support.\n"
                 "  - PERFORMANCE: Eliminated 16 redundant binary search iterations when Abney Correction is 0.0.\n"
                 "  - COMPATIBILITY: Added explicit VertexCount = 3 and PrimitiveTopology declarations.\n\n"
