// =================================================================================================
// Photoreal HDR Color Grader (V6.0.1 - Non-Riemannian Geodesic Spectral Edition)
// =================================================================================================
//
 ...
 * and Gamut Guard Soft-Knee Compression into a single coordinate round trip [1].
 * 
 * Incorporates:
 * - Adaptation 1: Dynamic Lightness-dependent Neutral Axis gray spine tracking [1].
 * - Adaptation 2: Non-Riemannian Diminishing Returns hyperbolic saturation scaling (Weber-Fechner) [1].
 * - Adaptation 3: Bezold-Brücke Geodesic Hue Correction (tracks curved trajectories instead of straight lines) [1].
 * - Adaptation 4: Exact Isoluminant Luminance Lock.
 */
float3 ApplyMBPurityAndGamutGuardLMS(float3 lms, float purity_scale, float knee, float abney_correction, float3 luma_LMS_coeffs, float3x3 to_RGB_boundary, float2 mb_white_static, float whitePt)
{
    // Fast bypass
    ...
    // Direct LMS-domain luma evaluation
    float luma = dot(lms, luma_LMS_coeffs);
    ...
    float2 mb_white = GetDriftedWhitePointMB(luma, whitePt, mb_white_static);
    ...
    float t_max = SolveGamutBoundaryExact(chroma_dir, luma_LMS_coeffs, to_RGB_boundary, mb_white);
...
void PS_PhotorealHDR(VS_Output input, out float4 fragColor : SV_Target)
{
    ...
    else if (iToneMapperMode == 2) // Non-Riemannian Geodesic Tone Mapper (NRG-TM)
    {
        ...
        // NRG-TM executes physiologically inside the LMS domain
        lms = ApplyNonRiemannianGeodesicToneMapper(lms, targetPeak, fCompressionStart, fDesaturationStrength, fConeResponseExponent, fHighlights, input.luma_LMS_coeffs, mb_white, whitePt);
    ...
    // ---------------------------------------------------------------------------------------------
    lms = ApplyMBPurityAndGamutGuardLMS(lms, fSaturation, fGamutGuardKnee, fAbneyCorrection, input.luma_LMS_coeffs, to_RGB_boundary, mb_white, whitePt);
...
technique PhotorealHDR_Mastering_V601 <
    ui_label = "Photoreal HDR V6.0.1 (Non-Riemannian Geodesic Spectral Edition)";
    ui_tooltip = "Reference-grade offline color grading executing the exact mathematics of non-Riemannian color space.\n\n"
                 "V6.0.1 changes:\n"
                 "  - Corrected the adaptive reference state in ApplyMConeCrosstalkLMS to evaluate cone-relative drive against the global adaptation state (diffuse scene white) rather than local pre-bleach nits.\n"
                 "  - Upgraded shoulder compression to the exact psychov_test20 generalized Naka-Rushton/Hill curve.\n"
                 "  - Fully integrated spectral M-cone crosstalk to prevent harsh clipping of highly saturated red and blue highlights.\n"
                 "  - Retained fully scale-invariant, division-free 24-step unrolled binary search boundary solver.\n"
                 "  - Dynamic white point drift alignment under the non-Riemannian framework.\n"
                 "  - Optimized Troland Bleaching reference white projections by leveraging Row-Sum matrix normalization.\n"
                 "  - Smoothed tone and creative contrast curves with a C1 continuous transition at the pivot, preventing slope cusps.\n"
                 "  - Implemented a strictly isoluminant luminance lock in the physiological chromaticity scaling stage.\n"
                 "  - Resolved dynamic scRGB working matrix boundaries to check Rec.2020 limit parameters.\n\n"
