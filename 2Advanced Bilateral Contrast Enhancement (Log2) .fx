 ...
 *
 * Version: 4.3.1 (Micro-optimized loop)
 */
...
{
    float CalculateWeight(const float r2,
...
void PS_BilateralContrast(float4 vpos : SV_Position, float2 texcoord : TEXCOORD0, out float4 fragColor : SV_Target)
{
    ...
    // Track local dynamic range for adaptive strength
    ...
    for (int y = -iRadius; y <= iRadius; ++y)
    {
        ...
        for (int x = -iRadius; x <= iRadius; ++x)
        {
            ...
            const float weight = BilateralFilter::CalculateWeight(
                r2, log2_ratio_center, log2_ratio_neighbor,
            ...
            if (weight > Constants::WEIGHT_THRESHOLD)
            {
                ...
                // Track local dynamic range (only for significant weights)
    ...
    if (sum_weight > Constants::WEIGHT_THRESHOLD)
    {
        ...
        // Apply shadow protection with smoothstep for perception-aligned gentle fade
